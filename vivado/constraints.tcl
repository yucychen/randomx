# =============================================================================
# constraints.tcl — Conditional (scripted) design constraints
# RandomX FPGA Framework — Xilinx Virtex UltraScale+ XCVU33P
#
# WHY THIS IS A .tcl AND NOT A .xdc
# ---------------------------------
# A .xdc file is read by Vivado in "constraint mode", which only accepts the
# XDC command subset. Tcl control-flow commands are rejected with
#   CRITICAL WARNING: [Designutils 20-1307] Command 'if' is not supported in
#   the xdc constraint file.
# and — worse than the warning — the guarded constraints are silently NOT
# applied, so the design ends up unconstrained (no create_clock, no clock
# groups, no I/O delays).
#
# Adding the very same code as a .tcl file in the constraints fileset makes
# Vivado source it as a full Tcl script, so `if` / `foreach` work as intended.
# vivado/build.tcl adds this file to constrs_1 with USED_IN
# {synthesis implementation}; vivado/constraints.xdc keeps the purely static
# constraints and stays the GUI target constraints file.
#
# Contents:
#   0. Resolve the system clock / reset port for either top module
#   1. Primary system clock (300 MHz)
#   2. Reset
#   3. HBM2 reference / APB clocks + asynchronous clock groups
#   4. Vendor-neutral AXI boundary I/O budget
#   5. SLR floorplanning (pblock template, disabled by default)
#
...
#
# NOTE: earlier revisions declared set_false_path on every m_axi_* port to
# silence the unconnected AXI boundary. Those exceptions have been removed on
# purpose — with HBM attached they would mask real violations on the
...
# ---------------------------------------------------------------------------
# 4. Vendor-neutral build only: the AXI4 master is still a device boundary, so
