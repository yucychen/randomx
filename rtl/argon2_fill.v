...
module argon2_fill #(
    ...
    // (RANDOMX_ARGON_MEMORY / RANDOMX_ARGON_ITERATIONS); the memory cost is
    // reduced to 8 blocks for simulation (`iverilog -DSIMULATION`).
`ifdef SIMULATION
    parameter ARGON_M = 8,          // memory blocks (1 KiB each)
`else
...
`endif
    ...
    input  wire [6:0]    key_len,  // key length in bytes (1..64)
...
localparam LANE_LEN      = ARGON_M / ARGON_LANES;
localparam SEG_LEN       = LANE_LEN / ARGON_SEGS;
...
localparam [1:0]  LAST_PASS     = ARGON_T - 1;
...
reg [3:0]  state;
reg [31:0] block_idx;   // current block index (0..ARGON_M-1)
...
wire [31:0] ref_area = (pass_cnt == 2'd0)
                     ? (block_idx - 32'd1)
...
wire [31:0] start_pos = (pass_cnt == 2'd0 || seg_cnt == 2'd3)
                      ? 32'd0
...
// ---------------------------------------------------------------------------
// H0 message block: Argon2 parameter encoding, at most 112 bytes ⇒ one block
// ---------------------------------------------------------------------------
wire [9:0]   key_bits  = {key_len, 3'b0};              // key_len * 8
wire [511:0] key_mask  = (key_len >= 7'd64) ? {512{1'b1}}
                                            : ((512'd1 << key_bits) - 512'd1);
wire [9:0]   tail_off  = 10'd224 + key_bits;           // 28 bytes + key
reg [1023:0] h0_msg;
...
always @* begin
    ...
    h0_msg[192 +: 32] = {25'b0, key_len};
    h0_msg[224 +: 512] = key & key_mask;
    h0_msg[tail_off +: 32] = ARGON_SALTLEN;              // LE32(|S|)
    h0_msg[(tail_off + 10'd32) +: 64] = ARGON_SALT;      // S
    h0_msg[(tail_off + 10'd96) +: 32] = 32'd0;           // LE32(|K|) = 0
    h0_msg[(tail_off + 10'd128) +: 32] = 32'd0;          // LE32(|X|) = 0
end

wire [127:0] h0_len = {121'b0, key_len} + 128'd48;   // 48 + key_len bytes
        ...
        case (state)
            ...
            ST_RD_REF: begin
                ...
                if (cache_rd_en && cache_rd_valid) begin
                    ...
                    rnd         <= 4'd0;
                    half        <= 1'b0;
                    state       <= (pass_cnt == 2'd0) ? ST_ROUNDS : ST_RD_CUR;
                end
            end
